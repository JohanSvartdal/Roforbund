...
public class ReceiveUserNameFromDb extends AbstractAppServlet {
    ...
    protected void writeBody(HttpServletRequest req, PrintWriter out) {
        ...
        UserModel userModel = new UserModel("Jan", "Erik", "janerik", "root");

        System.out.println("Forsoker a lage bruker");


        UserRepository userRepository = new UserRepository();
        userRepository.addUser(userModel, out);

        String nameFromDb = userRepository.getUserName(username,out);
        if (nameFromDb == null) {
            out.format("Fant ikke brukeren");
        }else {
